/**
 * Production-Ready Legal & Policy Management System
 * Defines data structures, default policy templates, HTML sanitization, and formatting.
 * 
 * Supports the 5 EXACT required policies:
 * 1. Terms & Conditions
 * 2. Privacy Policy
...
export interface LegalPage {
  id: string;
  page_key: "terms" | "privacy" | "refund-policy" | "disclaimer" | "contact-support";
  title: string;
  slug: string;
  short_description: string;
  content: string;           // Published live content visible to public
  draft_content?: string;     // Working draft content in admin editor
  meta_title: string;
...
export interface LegalAuditLog {
  id: string;
  admin_id: string;
  action: "CREATE" | "EDIT_DRAFT" | "PUBLISH" | "UNPUBLISH" | "RESTORE_VERSION";
  policy_key: string;
  page_title: string;
  previous_version: number | string;
...
/**
 * Robust HTML Sanitizer to prevent unsafe script injections, javascript: URIs, and rogue attributes
 */
export function sanitizeHTML(dirty: string): string {
  ...
  const forbiddenTags = [
    ...
    "base", "frame", "frameset", "meta", "link", "form", "input", "button"
  ];
  ...
  // Forbidden dangerous attributes
  ...
  allElements.forEach(el => {
    ...
    attributes.forEach(attr => {
      ...
      // Strip inline event listeners (onclick, onerror, onload, etc.)
...
/**
 * Format timestamp nicely for UI display
 */
...
/**
 * Generate default seed legal policies covering all required points
 */
export function getDefaultLegalPages(settings?: any): LegalPage[] {
